...
function determineOptionalityLevel(pos, tests)
{
    ...
    if (optionalityLevel) {
        if (pos == 0) {
            ...
            optionalityLevel = 0;
        } else if (!differentOptionalLevels) {
            optionalityLevel = 0;
        }
    }
...
function isSubsetOf(source, target, opts)
{
    ...
    if (source.match.def === target.match.nativeDef) {
        return true;
    }
            ...
            function handleQuantifier()
            {
                ...
                qndx++
                    ...
                    if (match) {
                        ...
                        if (breakloop) {
                            break; // search for next possible match
                        }
                        return true;
                    }
                }
