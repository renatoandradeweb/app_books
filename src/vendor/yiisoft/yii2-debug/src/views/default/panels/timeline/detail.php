...
<div class="debug-timeline-panel">
    <div class="debug-timeline-panel__header">
        <?php foreach ($dataProvider->getRulers() as $ms => $left) : ?>
            <span class="ruler"
                  style="margin-left: <?= StringHelper::normalizeNumber($left) ?>%"><b><?= sprintf(
                      '%.1f ms',
                      $ms
                                      ) ?></b></span>
        <?php endforeach; ?>
            ...
            <?php foreach ($models as $key => $model) : ?>
                ...
                if (!empty($model['memory'])) {
                    ...
                    if ($model['memoryDiff'] !== 0) {
                        $diff = ' title="' . (($model['memoryDiff'] > 0) ? '+' : '-') . sprintf(
                            '%.3f',
                ...
                <div class="debug-timeline-panel__item">
                    ...
                    <?php endif; ?>
                    <?= Html::tag('a', '
                        <span class="category">' . Html::encode($model['category']) . ' <span>' . sprintf(
                        '%.1f ms',
                        $model['duration']
                    ) . '</span>' . $memory . '</span>', [
                        'tabindex' => $key + 1,
