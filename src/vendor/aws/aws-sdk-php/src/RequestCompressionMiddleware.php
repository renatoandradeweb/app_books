...
class RequestCompressionMiddleware
{
    ...
    {
        if (
            isset($command['@request_min_compression_size_bytes'])
        ...
        if (
            !$this->shouldCompressRequestBody(
                $compressionInfo,
                $command,
                $operation,
                $request
    ...
    private function shouldCompressRequestBody(
        ...
        if ($compressionInfo) {
            if (
                isset($command['@disable_request_compression'])
                && $command['@disable_request_compression'] === true
            ) {
                return false;
            } elseif (
                $this->hasStreamingTraitWithoutRequiresLength($command, $operation)
            ) {
                return true;
            }
